...
class RplView(View):
    ...
    def get_all(cls, request) -> dict:
        ...
        context = {
            "rfids": [{"id": rfid.id, "name": rfid.name} for rfid in rfids],
            "lamps": [{"id": lamp.id, "name": lamp.name} for lamp in lamps],
            "buttons": [{"id": button.id, "name": button.name} for button in buttons],
        }
    ...
    def connect_buttons(cls, request: object, lamp: object) -> bool:
        try:
            ...
            sensor_buttons = request.user.sensor_set.filter(fun="btn")
            connected_buttons = set(
                [
                    sensor.id
                    for sensor in sensor_buttons
                    if sensor.button.lamp == lamp.ip
                ]
            )
            add_buttons = set([int(button_id) for button_id in get_data["btns"]])
