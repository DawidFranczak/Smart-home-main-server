...
const sensorSelect = (e) =>{
  ...
  switch (element.type) {
    case "radio":
      ...
      console.log()
      if (element.value === "uid" ||element.value === "card-uid" ) rfid.classList.add('Sensor-add__select__select-rfid--active');
...
const sensorSave = async () => {
  ...
  if (dataRep["response"] == "Udało sie dodać czujnik") {
    ...
    input.setAttribute("class", "sensor-del-button");
    ...
    pFun.setAttribute("class", "sensor-img");

    parent.setAttribute("class", "sensor");
    ...
    switch (newSensorFunction) {
      case "temp":
        ...
        document.querySelector("#add-sensor-temp-container").appendChild(parent);
        break;
      case "sunblind":
